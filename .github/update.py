...
def get_languages():
    with urllib.request.urlopen('https://raw.githubusercontent.com/vcmi/vcmi/beta/lib/texts/Languages.h') as f:
...
def create_md():
    ...
    for tool in list(tmp.values())[0].keys():
        df = pd.concat([df, pd.DataFrame({"Tool": "[" + tool + "](https://github.com/vcmi/vcmi/tree/beta/" + tool + "/translation)"} | {x:[format_value(tmp[x][tool]["ratio"])] if x in tmp else [format_value(0)] for x in languages_translate})], ignore_index=True)
    df = df.T.reset_index().T
    md.new_table(columns=df.shape[1], rows=df.shape[0], text=df.to_numpy().flatten(), text_align='center')
