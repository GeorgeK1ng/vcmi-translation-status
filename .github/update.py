...
def pad_header(text, min_len=15):
...
def create_md():
    ...
    df = pd.DataFrame({"Area": "[Main-Repo](https://github.com/vcmi/vcmi)"} | {
        padded: ([format_value(tmp[orig]["ratio"])] if orig in tmp else [format_value(0)])
